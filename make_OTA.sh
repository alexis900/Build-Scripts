...
./build/tools/releasetools/ota_from_target_files -i $ota_base/${SOURCE:n:2}*_$DEVICE-target_files-*.zip $newtarget ~/Builds/$(date +"%m-%d-%Y")/$SOURCE-$DEVICE-inc_update_$(date +"%Y%m%d").zip
