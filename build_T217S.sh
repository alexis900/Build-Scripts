...
echo -e "\E[1;32mBuilding T217S..."; tput sgr0
echo " "
echo " " >> ~/time.txt
echo " " >> ~/time.txt
echo "T217S build START time..." $(date +"%T") >> ~/time.txt
brunch $DEVICE | tee >(tail -3 > output.txt)
mail -s "T217S $SOURCE build status" "gr8nole@gmail.com" < output.txt

echo "T217S build STOP time...." $(date +"%T") >> ~/time.txt
echo " "
